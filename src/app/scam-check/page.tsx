...
export default function ScamCheckPage() {
  ...
  return (
    <PageContainer>
      <div className="max-w-3xl mx-auto py-10 space-y-8 px-4 sm:px-0">
        {/* Header */}
        ...
        {/* Search Box */}
        <Card className="p-2 border-slate-200/80 bg-white shadow-xs rounded-2xl">
          <form onSubmit={handleSearch} className="flex gap-2 p-2">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-11 h-12 text-sm sm:text-base rounded-xl border-slate-200 bg-slate-50 text-slate-900 focus:ring-2 focus:ring-emerald-500"
              />
            </div>
            <Button 
              type="submit" 
              ...
              className="h-12 px-6 font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-xs"
            >
        ...
        <div className="flex items-center justify-center gap-2 text-xs text-slate-400">
          ...
          <span className="font-mono text-slate-600 bg-slate-100 px-2 py-0.5 rounded cursor-pointer hover:bg-slate-200 transition-colors" onClick={() => setQuery('campusleader')}>@campusleader</span>
        </div>
        ...
        {searched && (
          <div className="pt-2 animate-in fade-in duration-300">
            {loading ? (
              ...
              <Card className="overflow-hidden border border-slate-200/80 rounded-3xl shadow-xs bg-white">
                <div className={`h-2.5 ${isVerified ? 'bg-gradient-to-r from-emerald-500 to-teal-600' : 'bg-gradient-to-r from-amber-400 to-orange-500'}`} />
                
                <CardContent className="p-6 sm:p-8 space-y-6">
                  {/* Top Profile Header */}
                  <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="h-16 w-16 rounded-2xl bg-slate-100 flex items-center justify-center shrink-0 border border-slate-200 overflow-hidden shadow-inner">
                        {result.avatar_url ? (
                    ...
                    <div>
                      {isVerified ? (
                        <div className="flex items-center gap-1.5 text-emerald-800 bg-emerald-100 border border-emerald-300 px-3 py-1.5 rounded-full text-xs font-bold shadow-xs">
                          <ShieldCheck className="w-4 h-4 text-emerald-600" />
                          <span>CUSTECH Verified Student</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1.5 text-amber-800 bg-amber-100 border border-amber-300 px-3 py-1.5 rounded-full text-xs font-bold shadow-xs">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
                    <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-100">
                      ...
                      <span className="text-lg font-black text-slate-900 capitalize mt-0.5 block">
                        {result.trust_level?.replace('_', ' ') || 'Registered'}
                      </span>
                    </div>

                    <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-100">
                      <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block">Completed Deals</span>
                    ...
                    <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-100">
                      ...
                      <span className="text-lg font-black text-amber-600 flex items-center gap-1 mt-0.5">
                        ...
                        {Number(result.rating_avg || 5.0).toFixed(1)} <span className="text-xs text-slate-400 font-normal">({result.rating_count || 0})</span>
                      </span>
                    </div>
                  ...
                  <div className="pt-2 flex justify-end">
                    ...
                    >
                      <Button className="rounded-xl font-bold text-xs gap-2 bg-slate-900 text-white hover:bg-slate-800">
              ...
              <Card className="p-8 text-center space-y-3 rounded-2xl border-dashed border-2 border-slate-200 bg-white">
                <AlertTriangle className="w-10 h-10 text-amber-500 mx-auto" />
                <h3 className="font-bold text-base text-slate-900">No Member Found</h3>
                <p className="text-xs text-slate-500 max-w-sm mx-auto">
                  We couldn't find any member with username or name matching <span className="font-mono font-bold text-slate-700">"{query}"</span>. If someone claims to be on CUSTECH Marketplace, verify their username spelling carefully.
                </p>
              </Card>
            )}
          </div>
